  ...
  serverCall(otpData: verifyOtp) {
    this.userService.verifyOtp(otpData).subscribe({
      next: (Response) => {
        ...
        this.messageToaster.showSuccessToastr(Response.message);
        // Check if OTP verification is for forgot password
      const isForgotPassword = localStorage.getItem('isForgotPassword');

      if (isForgotPassword) {
        localStorage.removeItem('isForgotPassword'); 
        localStorage.setItem('email', this.email as string);// Remove flag after use
        this.router.navigate(['new_password']); // Navigate to the new password page
      } else {
